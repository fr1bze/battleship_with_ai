...
class Game:
    def __init__(self,human1, human2) -> None:
        self.human1 = human1
        self.human2 = human2
        self.player1 = Player()
        self.player2 = Player()
    ...
    def basic_ai(self): 
        ...
        # search in adjacent cells of hits
        ...
        #pick 'U' square with direct level-2 neighbor marked as 'H'
        for u in unknown:
            if u in unknown_with_neighboring_hits1 and u in unknown_with_neighboring_hits2:
                self.make_move(u)
                return
            
        # pick 'U' square that has the neighbor level-1 marked as 'H' and level-2 marked as 'H'  
        ...
        for u in unknown:
            ...
            col = u % 10
            if (row + col) % 2 == 0:
                check_board.append(u)
        if len(check_board) > 0:
            self.make_move(random.choice(check_board))                        
            return
